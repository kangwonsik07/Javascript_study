...
const options = {
   ...
   headers: {
      ...
      // 보안을 위해서 서버에게 주는 인증키
      Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1MjVhNzIwYzZkODU1YTUzOGU5MzA0MjZmMTY1ZjYyMCIsIm5iZiI6MTcyOTgyNTUwOC43NTQxMTEsInN1YiI6IjY3MWFlNzc5YjNkNWNiYjg0MmYzZjdjZSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.uiV42prRI-s3VWYddTyZaB8XbmI-tXnfMTG2N2F9C8A',
   },
}

// fetch: 서버에 request를 요청하는 자바스크립트에서 제공하는 함수
// fetch(request 주소, request 할때 서버에 같이 전달하는 옵션)
// 물음표 뒤는 쿼리스트림: 서버에 보내는 값들

// 왜 fetch는 promise를 사용할까?(왜 비동기일까?)
// promise, async, await -> 비동기
// request 해주는 과정을 비동기로 동작시키고 있다

// 서버에 장애가 있어나 네트워크 문제가 있거나 했을때 동기적으로 실행이 된다면 사용자는 다른 작업을 할 수 X ->따라서 reauest 작업은 비동기로 진핸된다.

const url = 'https://api.themoviedb.org/3/movie/now_playing?language=ko-KR&page=1&region=KR'

const getPlayingMovies = async (url) => {
   try {
      const response = await fetch(url, options) //promise객체를 리턴 ,서버에서 데이터를 가져올때까지 기다린다
      //   console.log(response) // 기다렸다가 다 가지고 오면 실행

      const data = await response.json() // await 를  지정하는 이우: fetch는 비동기적으로 실행되므로 서버에서 request 해오는 딜레시간 중에 실행 된다.
